...
class GroupEditActivity : AppCompatActivity() {
    ...
    fun leaveGroup() {
        ...
        call.enqueue(object : Callback<Default> {
            override fun onResponse(call: Call<com.example.everytask.models.response.Default>, response: Response<com.example.everytask.models.response.Default>) {
                if (response.isSuccessful) {
                    finish()
                }
            }

            override fun onFailure(call: Call<com.example.everytask.models.response.Default>, t: Throwable) {
                Toast.makeText(this@GroupEditActivity, "No connection to server", Toast.LENGTH_SHORT).show()
            }
        })
    }
